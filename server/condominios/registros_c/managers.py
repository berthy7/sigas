...
class Registros_cManager(SuperManager):
    ...
    def filtrar(self, fechainicio, fechafin,usuario):
        ...
        if usuario.sigas:
            registros = self.db.query(self.entity).filter(func.date(self.entity.time).between(fechainicio, fechafin)).order_by(self.entity.id.desc()).limit(20).all()
        else:
            registros = self.db.query(self.entity).join(Dispositivo).filter(Dispositivo.fkcondominio == usuario.fkcondominio).filter(func.date(self.entity.time).between(fechainicio, fechafin)).order_by(self.entity.id.desc()).limit(20).all()
    ...
    def listar_todo_diccionario(self,usuario):
        ...
        if usuario.sigas:
            registros = self.db.query(self.entity).filter(self.entity.time.cast(Date) == fechahoy).order_by(
                self.entity.time.cast(Date).asc(), self.entity.time.cast(Time).asc()).limit(20).all()
        else:
