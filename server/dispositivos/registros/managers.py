...
class RegistrosManager(SuperManager):
    ...
    def listar_todo_diccionario(self):
        ...
        registros = self.db.query(self.entity).filter(self.entity.time.cast(Date) == fechahoy).order_by(self.entity.time.cast(Date).asc(),self.entity.time.cast(Time).asc()).all()
